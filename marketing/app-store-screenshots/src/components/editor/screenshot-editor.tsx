...
export function ScreenshotEditor() {
  ...
  async function exportAll() {
    ...
    const saveToDisk =
      ...
      new URLSearchParams(window.location.search).has("saveExports");
    let diskCount = 0;

    if (saveToDisk) {
      // Clear previous exports for this platform before writing new ones.
      try {
        await fetch("/api/export-save", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clearDir: platform, files: [] }),
        });
      } catch {
      ...
      try {
        await fetch("/api/export-save", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clearDir: `framed/${state.device}`, files: [] }),
        });
      } catch {
        /* ignore */
      }
    ...
    async function persistDisk(files: { relativePath: string; base64: string }[]) {
      ...
      diskCount += json.count || files.length;
    }
      ...
      for (const size of sizes) {
        for (let i = 0; i < currentSlides.length; i++) {
          ...
          try {
            ...
            if (saveToDisk) {
              const batch = [{ relativePath: path, base64 }];
              if (size === sizes[0]) {
                batch.push({
                  relativePath: `framed/${state.device}/${locale}/${filename}`,
                  base64,
                });
              }
              await persistDisk(batch);
            }
            okCount += 1;
          } catch (e) {
            failed += 1;
    ...
    if (okCount > 0 && saveToDisk) {
      toast.success(`Saved ${diskCount} PNGs to exports/`);
      if (typeof window !== "undefined") {
        (window as unknown as { __EXPORT_DISK_DONE__?: boolean }).__EXPORT_DISK_DONE__ = true;
        (window as unknown as { __EXPORT_DISK_COUNT__?: number }).__EXPORT_DISK_COUNT__ = diskCount;
      }
    }
