...
function Caption({
  ...
  const isFitRpg = theme.id === "fitrpg-dark-fantasy";
  ...
  const labelSize = unit * (isFitRpg ? 0.026 : 0.028);
  const headlineSize = unit * (isFitRpg ? 0.086 : 0.092);
  return (
    <div style={{ textAlign: align, position: "relative", width: "100%" }}>
      <EditableText
        ...
        style={{
          fontFamily: isFitRpg
            ? "var(--font-fitrpg-display), Cinzel, serif"
            : undefined,
          fontSize: labelSize,
          fontWeight: isFitRpg ? 700 : 600,
          letterSpacing: unit * (isFitRpg ? 0.0045 : 0.0015),
          color: accent,
          textTransform: "uppercase",
          marginBottom: unit * (isFitRpg ? 0.022 : 0.018),
          minHeight: unit * 0.03,
          textShadow: isFitRpg ? "0 2px 18px rgba(0,0,0,0.55)" : undefined,
        }}
      />
      <EditableText
        ...
        style={{
          fontFamily: isFitRpg
            ? "var(--font-fitrpg-sans), Exo 2, system-ui, sans-serif"
            : undefined,
          fontSize: headlineSize,
          fontWeight: isFitRpg ? 800 : 700,
          lineHeight: isFitRpg ? 0.94 : 0.96,
          letterSpacing: -unit * (isFitRpg ? 0.0018 : 0.001),
          color: fg,
          textShadow: isFitRpg
            ? "0 4px 28px rgba(0,0,0,0.65), 0 1px 0 rgba(255,255,255,0.06)"
            : undefined,
        }}
      />
      {isFitRpg ? (
        <div
          aria-hidden
          style={{
            marginTop: unit * 0.028,
            width: unit * 0.09,
            height: Math.max(3, unit * 0.0045),
            borderRadius: 999,
...
function backgroundFor(theme: Theme, inverted?: boolean) {
  // FitRPG: cinematic void — controlled violet + ember, gold is for type not flood fill
  if (theme.id === "fitrpg-dark-fantasy") {
    if (inverted) {
      return [
        "radial-gradient(ellipse 95% 55% at 50% -8%, #7C1CFF33 0%, transparent 55%)",
        "radial-gradient(ellipse 80% 55% at 110% 105%, #EF444455 0%, transparent 52%)",
        "radial-gradient(ellipse 55% 40% at -5% 80%, #D4A01722 0%, transparent 50%)",
        `linear-gradient(168deg, ${theme.bgAlt} 0%, #1C0610 48%, #070208 100%)`,
      ].join(", ");
    }
    return [
      "radial-gradient(ellipse 100% 60% at 18% -12%, #8B2CFF2E 0%, transparent 52%)",
      "radial-gradient(ellipse 70% 48% at 98% 92%, #DC262644 0%, transparent 48%)",
      "radial-gradient(ellipse 45% 35% at 70% 20%, #D4A01718 0%, transparent 55%)",
      `linear-gradient(168deg, ${theme.bg} 0%, #0A0614 42%, #030108 100%)`,
    ].join(", ");
  }
  if (inverted) {
...
function SlideBackground({
  ...
  if (isFitRpg) {
    return (
      ...
      >
        {/* Soft atmospheric lights — restrained, not purple flood */}
        <Blob
          cW={cW}
          color={inverted ? "#EF4444" : "#7C3AED"}
          x={inverted ? 55 : -18}
          y={inverted ? 62 : -12}
          size={inverted ? 58 : 62}
          opacity={inverted ? 0.16 : 0.18}
        />
        <Blob
          cW={cW}
          color={inverted ? "#7C3AED" : "#EF4444"}
          x={inverted ? -20 : 68}
          y={inverted ? -8 : 72}
          size={48}
          opacity={0.12}
        />
        <Blob cW={cW} color="#D4A017" x={38} y={42} size={28} opacity={0.07} />
        {/* Vignette */}
        <div
          aria-hidden
          style={{
        ...
        <div
          ...
          style={{
            ...
            height: cH * 0.18,
            background: inverted
              ? "radial-gradient(ellipse at center, rgba(239,68,68,0.22) 0%, transparent 70%)"
...
function SlideElements({
  ...
  function renderDevice(id: "device" | "deviceSecondary", rect: Rect, src: string, extraStyle?: React.CSSProperties) {
    ...
    const fitRpgGlow =
      theme.id === "fitrpg-dark-fantasy"
        ? {
            filter:
              id === "deviceSecondary"
                ? "drop-shadow(0 28px 48px rgba(0,0,0,0.7))"
                : "drop-shadow(0 36px 70px rgba(0,0,0,0.78)) drop-shadow(0 0 42px rgba(124,58,237,0.22))",
          }
        : {};
    return (
      <Movable
      ...
      >
        <Frame
          src={src}
          hideEmpty={hideEmpty}
          style={{ width: "100%", height: "100%", ...fitRpgGlow, ...extraStyle }}
        />
      </Movable>
    );
  }
